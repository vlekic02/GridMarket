...
public class UserListener implements ListenerAdapter {
  ...
  @Override
  public void onUsernameChangeEvent(UsernameChangeEvent event) {
    LOGGER.debug("Received username change event: {}", event);
    userService.changeUsername(event.oldUsername(), event.newUsername());
  }
}
